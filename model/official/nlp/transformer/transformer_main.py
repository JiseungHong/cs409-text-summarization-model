...
def translate_and_not_compute_bleu(model,
                               params,
                               bleu_source,
                               bleu_ref,
                               vocab_file,
  ...
  translate.translate_file_and_print(
      model,
      params,
      subtokenizer,
      bleu_source,
...
#    params["save_weights_only"] = flags_obj.save_weights_only
  ...
  def eval(self):
    ...
    with distribute_utils.get_strategy_scope(distribution_strategy):
      ...
      # todo: check whether ckpt is set.
      # put model file into tensorboard. --logdir = model_extend_data
      # server host 6006 -> test loss
      self._load_weights_if_possible(
          self.predict_model,
          tf.train.latest_checkpoint(self.flags_obj.model_dir))
      # self.predict_model.summary()
      
    # todo: set flags properly. bleu_source, bleu_ref = source data(wmt32k-dev.lang1), target data(wmt32k-dev.lang1)
    return translate_and_not_compute_bleu(
  ...
  def predict(self):
    ...
    with tf.name_scope("model"):
      ...
      # todo: call .ckpt - print and debugging!!!
      self._load_weights_if_possible(
          model, tf.train.latest_checkpoint(self.flags_obj.model_dir))
      # model.summary()
    subtokenizer = tokenizer.Subtokenizer(flags_obj.vocab_file)
    # todo: check whether ds is properly set.
    ds = data_pipeline.eval_input_fn(params)
    ds = ds.map(lambda x, y: x).take(_SINGLE_SAMPLE)
    # for a, b in ds.take(_SINGLE_SAMPLE):
      # print (a)
      # print (b)
    ...
    with open("input.csv", 'w', newline='') as csvfile:
      ...
      for i in range(length):
          translation = translate.translate_from_input(val_outputs[i], subtokenizer)
